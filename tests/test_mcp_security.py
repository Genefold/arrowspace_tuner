...
def test_relative_input_path_rejected(ctx) -> None:
    with pytest.raises(Exception, match="absolute"):
        ensure_allowed_path("embeddings.npy", allowed_roots=ctx.allowed_roots,
                            must_exist=True)


def test_path_outside_allowed_root_rejected(ctx, tmp_path) -> None:
    outside = tmp_path.parent / "outside.npy"
    np.save(outside, np.ones((10, 4)))
    try:
        with pytest.raises(Exception, match="outside"):
            ensure_allowed_path(str(outside), allowed_roots=ctx.allowed_roots,
                                must_exist=True)
    finally:
        outside.unlink(missing_ok=True)


def test_symlink_escaping_allowed_root_rejected(ctx, npy_file, tmp_path) -> None:
    link = npy_file.parent / "escape.npy"
    link.symlink_to(tmp_path / "elsewhere.npy")
    try:
        with pytest.raises(Exception, match="exist|outside"):
            ensure_allowed_path(str(link), allowed_roots=ctx.allowed_roots,
                                must_exist=True)
    finally:
        link.unlink(missing_ok=True)


def test_nonexistent_input_rejected(ctx, npy_file) -> None:
    with pytest.raises(Exception, match="not accessible"):
        ensure_allowed_path(
            str(npy_file.parent / "missing.npy"),
            allowed_roots=ctx.allowed_roots, must_exist=True,
        )


def test_url_like_input_rejected(ctx) -> None:
    for url in (
        ...
        with pytest.raises(Exception, match="URL"):
            ensure_allowed_path(url, allowed_roots=ctx.allowed_roots,
                                must_exist=True)


def test_pickle_file_rejected(ctx, tmp_path) -> None:
    ...
    try:
        with pytest.raises(Exception, match="Unsupported file type"):
            ensure_allowed_path(str(pkl), allowed_roots=ctx.allowed_roots,
                                must_exist=True)
    finally:
        pkl.unlink(missing_ok=True)


def test_object_array_npy_rejected(ctx) -> None:
    root = Path(ctx.allowed_roots[0])
    path = root / "object.npy"
    np.save(path, np.array([[object()], [object()]], dtype=object),
            allow_pickle=True)
    try:
...
def test_report_path_outside_allowed_root_rejected(ctx, tmp_path) -> None:
    outside = tmp_path.parent / "outside_reports_mcp"
    with pytest.raises(Exception, match="outside"):
        ensure_allowed_report_dir(str(outside), allowed_roots=ctx.allowed_roots)


def test_report_path_relative_rejected(ctx) -> None:
    with pytest.raises(Exception, match="absolute"):
...
def test_max_file_size_enforced(ctx, npy_file) -> None:
    tight = McpContext(
...
def test_max_trials_enforced(ctx, npy_file) -> None:
    tight = McpContext(
        allowed_roots=ctx.allowed_roots,
...
def test_max_n_jobs_enforced(ctx, npy_file) -> None:
    tight = McpContext(
        allowed_roots=ctx.allowed_roots,
...
def test_limits_defaults() -> None:
    assert load_limits({}) == McpLimits(
        max_input_bytes=2 * 1024**3, max_trials=100, max_n_jobs=4
    )
...
def test_limits_reject_non_integer() -> None:
    ...
    with pytest.raises(McpConfigurationError):
        load_limits({"ARROWSPACE_TUNER_MAX_TRIALS": "many"})
...
def test_report_dir_inside_allowed_root_is_accepted(ctx, tmp_path) -> None:
    root = Path(ctx.allowed_roots[0])
    report_dir = root / "reports"
    resolved = ensure_allowed_report_dir(str(report_dir),
                                         allowed_roots=ctx.allowed_roots)
    assert resolved == str(report_dir.resolve())
    assert not report_dir.exists()  # creation is deferred to the service


def test_existing_report_dir_is_not_overwritten(ctx, tmp_path) -> None:
    root = Path(ctx.allowed_roots[0])
    existing = root / "existing_reports"
    existing.mkdir()
    try:
        with pytest.raises(Exception, match="already exists"):
            ensure_allowed_report_dir(str(existing),
                                      allowed_roots=ctx.allowed_roots)
    finally:
        existing.rmdir()